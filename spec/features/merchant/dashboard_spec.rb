...
RSpec.describe 'Merchant Employee Dashboard' do
  ...
  it "shows pending orders containing items sold by the merchant" do
    ...
    merchant = dog_shop.users.create!(name: 'Jim', address: '456 Blah Blah Blvd', city: 'Denver', state: 'CO', zip: '12345', email: 'regularjim@me.com', password: 'alsosecret', role: 1)
    ...
    order_pending = Order.create!(name: 'Meg', address: '123 Stang Ave', city: 'Hershey', state: 'PA', zip: 17033)
    order_fulfilled = Order.create!(name: 'Yo', address: 'Whatever', city: 'Place', state: 'PA', zip: 17033)
    ...
    order_fulfilled.item_orders.create!(item: light_roast, price: light_roast.price, quantity: 4, status: 'Pending')
    order_fulfilled.reload

    visit '/merchant'

    expect(page).to_not have_content(order_fulfilled.id)

    within "#merchant_orders-#{order_pending.id}" do
